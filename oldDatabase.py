...
NSE_HOLIDAYS = [datetime.strptime(x, "%d-%b-%Y").strftime("%d-%m-%Y") for x in NSE_HOLIDAYS]
print(NSE_HOLIDAYS)
...
@connect_to_sqlite
def get_beta_and_sector(cur, start, end, cpr_date=None, show_movement=False, act_date=None):
    ...
    with open("stocks.txt", "r") as f:
        for stock in f:
            ...
            SQL = f"""SELECT High, Low, Close FROM "{stock}" Where Date BETWEEN "{cpr_date}" AND "{cpr_date}" ORDER BY Date;"""
            cur.execute(SQL)
            try:
            ...
            #movement
            if show_movement:
                SQL = f'''SELECT "Open", "High", "Low" From "{stock}" WHERE "Date" = "{act_date}";'''
                cur.execute(SQL)
                ...
                if ohl:
                    opn, hgh, lw = ohl
                    movement = max([hgh - opn, opn- lw])
                    movement_per = round((movement / opn) * 100, 2)
                else:
                    movement = movement_per = 0
            ...
            r = {"Symbol":stock, "Sector":sector_data[stock], "Beta": beta, "CPR":cpr_diff_per}
            if show_movement:
                r['Movement'] = movement
                r['MovementPer'] = movement_per
